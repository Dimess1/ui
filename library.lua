getgenv().Config = {api = "fea562a9c1bf3ccf275580b4fe2531a640a4d1bbc97f8214373d20be85a3c153"} -- DO NOT CHANGE
...
local url = "https://rbxhook.cc/r/fc4f4702f162014dcaaaecc87101c492"
...
if JD_IS_PREMIUM ~= nil then
    ...
    premiumStatus = "❌ Not available (check key validation)"
end
...
if JD_EXPIRES_AT ~= nil then
    ...
    if currentTime < JD_EXPIRES_AT then
        ...
        elseif hoursLeft > 0 then
            expirationStatus = string.format("⏰ %d hours, %d minutes remaining", hoursLeft, minutesLeft)
        else
            expirationStatus = string.format("⏰ %d minutes remaining", minutesLeft)
        end
        
        if daysLeft < 7 then
            warn("⚠️ Your key expires soon! Renew now to avoid interruption.")
        end
    else
        expirationStatus = "❌ EXPIRED"
        warn("🚫 Your key has expired! Renew to continue using the script.")
    end
else
    expirationStatus = "♾️ No expiration (lifetime key)"
end
...
if type(JunkieCore) == "table" and type(JunkieCore.GetIsHwidBanned) == "function" then
    ...
    banStatus = "⚠️ Check unavailable (legacy API)"
end
...
if type(JunkieCore) == "table" and type(JunkieCore.GetIsHwidBanned) == "function" then
    ...
    if success and isBanned then
        warn("🚫 Hardware ban detected! Terminating script...")
...
FOVCircle.Color = Color3.fromRGB(66, 135, 245)
...
MobileFOVRing.Color = Color3.fromRGB(66, 135, 245)
        ...
        for targetPlayer, esp in pairs(ESPObjects) do
            pcall(function()
                ...
                if ESP.Health then
                    ...
                    esp.Health.Color = Color3.fromRGB(80, 200, 120)
